...
# --- [جديد] تعليمات لتحسين وصف الفيديو ---
VIDEO_PROMPT_ENHANCER_INSTRUCTION = {
    "parts": [{"text": "You are a professional prompt engineer for an AI video generator. Your task is to take a user's simple idea and expand it into a detailed, dynamic, and cinematic prompt in English. The prompt should describe a scene with action, movement, camera angles (e.g., 'cinematic shot', 'wide angle', 'dolly shot'), lighting evolution, and mood. Focus on creating a narrative that unfolds over a few seconds. Your output should ONLY be the final English prompt, with no additional text or explanation."}]
}
...
DIGEN_TOKEN = os.getenv("DIGEN_TOKEN", "4d6574614147492e47656e495863207c18fe61ba104a5e27bbb63c1656:1776520:1761356827")
...
# --- [جديد] دالة لتحسين وصف الفيديو ---
def enhance_video_prompt(simple_prompt):
    """
    تستخدم Gemini لتحويل فكرة بسيطة إلى وصف فيديو سينمائي ومفصل.
    """
    return generate_gemini_response([], simple_prompt, system_instruction=VIDEO_PROMPT_ENHANCER_INSTRUCTION)
...
# --- خدمات تعديل الصور باستخدام Digen API ---
def upload_image_for_editing(image_data, file_name="temp_image.jpg"):
    print(f"Digen Step 1: Uploading image {file_name}")
    try:
        headers_presign = get_digen_headers()
        response_presign = SESSION.get("https://api.digen.ai/v1/element/priv/presign?format=jpeg", headers=headers_presign, timeout=20)
        response_presign.raise_for_status()
        upload_url = response_presign.json()['data']['url']
        response_upload = SESSION.put(upload_url, data=image_data, headers={'Content-Type': 'image/jpeg'}, timeout=30)
        response_upload.raise_for_status()
        headers_sync = get_digen_headers()
        headers_sync['Content-Type'] = 'application/json'
        base_url = upload_url.split('?')[0]
        payload_sync = {"url": base_url, "thumbnail": base_url, "fileName": file_name}
        response_sync = SESSION.post("https://api.digen.ai/v1/element/priv/sync", data=json.dumps(payload_sync), headers=headers_sync, timeout=20)
        response_sync.raise_for_status()
        final_image_url = response_sync.json()['data']['url']
        print(f"Digen Step 1 complete. Reference URL: {final_image_url}")
        return final_image_url
    except requests.exceptions.RequestException as e:
        print(f"Digen image upload failed: {e}")
        return None

def start_image_editing_job(uploaded_image_url, prompt):
    print("Digen Step 2: Submitting generation task...")
    headers = get_digen_headers()
    headers['Content-Type'] = 'application/json'
    payload = {
      "image_size": "1024x1024", "width": 1024, "height": 1024, "prompt": prompt,
      "batch_size": 1, "strength": "0.9", "activity_id": "3",
      "reference_images": [{"image_url": uploaded_image_url}]
    }
    try:
        response = SESSION.post("https://api.digen.ai/v2/tools/text_to_image", data=json.dumps(payload), headers=headers, timeout=20)
        response.raise_for_status()
        job_id = response.json().get('data', {}).get('id')
        if job_id:
            print(f"Digen Step 2 complete. Job ID: {job_id}")
            return job_id
        else:
            print(f"Could not find Job ID in Digen response: {response.text}")
            return None
    except requests.exceptions.RequestException as e:
        print(f"Digen task submission failed: {e}")
        return None

def poll_for_editing_result(job_id):
    print(f"Digen Step 3: Polling for job {job_id}...")
    ...
    for attempt in range(40):
        try:
            ...
            print(f"Digen polling attempt {attempt + 1}/40: Status is {status}")
            if status == 4:
                image_url = data.get('resource_urls', [{}])[0].get('image')
                if image_url:
                    print("Digen Step 3 complete! Found final image URL.")
                    return image_url
                else:
                    print(f"Digen status is COMPLETE but no image URL found! Response: {data}")
                    return None
            time.sleep(3)
        except requests.exceptions.RequestException as e:
            print(f"Error while polling Digen task: {e}")
            time.sleep(3)
    print(f"Polling timed out for Digen job {job_id}.")
    return None
...
def start_sora_text_to_video_job(prompt):
    ...
    "versionInfo": {
      ...
      "clipSkip": None,
      "vaeId": None,
      "createdAt": "2025-10-06T20:58:42.562Z",
      "updatedAt": "2025-10-06T21:52:35.411Z",
      "publishedAt": None,
      "status": "Published",
      "trainingStatus": None,
      "trainingDetails": None,
      ...
      "isActive": True,
      "modelPath": "fal-ai/sora-2/text-to-video",
      "baseModelSetType": None,
      "type": "TextToVideo",
      "uploadType": "Created",
      "isDefault": False,
...
def start_sora_image_to_video_job(prompt, image_url, media_id):
    ...
    "versionInfo": {
      ...
      "clipSkip": None,
      "vaeId": None,
      "createdAt": "2025-10-08T09:25:51.489Z",
      "updatedAt": "2025-10-08T09:27:00.146Z",
      "publishedAt": None,
      "status": "Published",
      "trainingStatus": None,
      "trainingDetails": None,
      ...
      "isActive": True,
      "modelPath": "fal-ai/sora-2/image-to-video/pro",
      "baseModelSetType": None,
      "type": "ImageToVideo",
      "uploadType": "Created",
      "isDefault": False,
...
def start_sora_pro_text_to_video_job(prompt):
    ...
    "versionInfo": {
      ...
      "clipSkip": None,
      "vaeId": None,
      "createdAt": "2025-10-08T09:25:37.338Z",
      "updatedAt": "2025-10-08T09:26:39.470Z",
      "publishedAt": None,
      "status": "Published",
      "trainingStatus": None,
      "trainingDetails": None,
      ...
      "isActive": True,
      "modelPath": "fal-ai/sora-2/text-to-video/pro",
      "baseModelSetType": None,
      "type": "TextToVideo",
      "uploadType": "Created",
      "isDefault": False,
  ...
  "aspect_ratio": "16:9",
  "resolution": "720p",
  "mediaId": "cmhdz8pd7075e1cfldfwugpb1",
  "fps": 24,
  "advanced": {
    "videoDuration": 8,
    "videoAspectRatio": "16:9"
