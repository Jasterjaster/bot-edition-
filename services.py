...
# --- [تحديث] تعليمات مخصصة لكل وظيفة ---

# 1. لإنشاء الصور من نص فقط
PROMPT_ENHANCER_FOR_IMAGE_GEN_INSTRUCTION = {
    "parts": [{"text": "You are a professional prompt engineer for an AI image generator. Your task is to take a user's simple idea and expand it into a detailed, rich, and effective prompt in English for creating a static image. Focus on visual details: subject, environment, lighting, artistic style (e.g., photography, illustration, 3D render), composition, and technical specs like '8K, photorealistic, sharp focus'. Do not describe actions or movements over time. Your output should ONLY be the final English prompt."}]
}

# 2. لإنشاء فيديو من نص فقط
PROMPT_ENHANCER_FOR_VIDEO_TEXT_INSTRUCTION = {
    "parts": [{"text": "You are a professional prompt engineer for an AI video generator. Your task is to take a user's simple text idea and create a detailed prompt in English for a video scene. Describe the setting, the subject, and crucially, the ACTIONS and CAMERA MOVEMENTS over a short duration. For example, instead of 'a man walking', write 'A medium shot of a man in a trench coat walking purposefully down a rain-slicked neon-lit alley, camera follows him smoothly'. Your output should ONLY be the final English prompt."}]
}

# 3. لإنشاء فيديو من صورة ونص
PROMPT_ENHANCER_FOR_VIDEO_WITH_IMAGE_INSTRUCTION = {
    "parts": [{"text": "You are a professional prompt engineer for an AI video generator. Your task is to analyze the provided image and the user's simple text idea. Create a detailed prompt in English that animates the contents of the image according to the user's idea. Describe motion, camera movement (pan, zoom, tilt), and how elements within the static image come to life. For example, if the image is a car and the user says 'drive away', you describe 'The car's wheels begin to spin, smoke puffs from the exhaust, and it smoothly accelerates out of the frame to the right, camera holds steady'. Your output should ONLY be the final English prompt."}]
}

# 4. لتعديل صورة بناء على صورة ونص
PROMPT_ENHANCER_FOR_EDIT_INSTRUCTION = {
    "parts": [{"text": "You are a professional prompt engineer for an AI image editor. Your task is to analyze the provided original image and the user's text instruction for modification. Combine them into a clear, concise, and effective instruction in English for an AI. The prompt should state the desired change directly. Examples: 'Change the subject's shirt color to red', 'Add a futuristic city in the background', 'Apply a vintage, black and white photographic style to the entire image', 'Make the cat wear a wizard hat'. Your output should ONLY be the final English prompt."}]
}
...
BASEDLABS_HEADERS = {
    ...
    # --- [ تم التحديث ] --- تم تحديث الكوكي لتصحيح مشكلة إنشاء الصور
...
def generate_enhanced_prompt(simple_prompt):
    return generate_gemini_response([], simple_prompt, system_instruction=PROMPT_ENHANCER_FOR_IMAGE_GEN_INSTRUCTION)

def generate_enhanced_video_text_prompt(simple_prompt):
    return generate_gemini_response([], simple_prompt, system_instruction=PROMPT_ENHANCER_FOR_VIDEO_TEXT_INSTRUCTION)

def generate_enhanced_video_with_image_prompt(simple_prompt, image_base64):
    return generate_gemini_response([], simple_prompt, image_base64=image_base64, system_instruction=PROMPT_ENHANCER_FOR_VIDEO_WITH_IMAGE_INSTRUCTION)

def generate_enhanced_edit_prompt_with_image(simple_prompt, image_base64):
    return generate_gemini_response([], simple_prompt, image_base64=image_base64, system_instruction=PROMPT_ENHANCER_FOR_EDIT_INSTRUCTION)
...
def generate_image_from_prompt(prompt, image_count=1):
    ...
    try:
        print(f"BasedLabs: Starting image generation for {image_count} image(s)...")
        ...
        print(f"BasedLabs: Job started with request_id: {request_id}. Polling for results...")
        poll_url = f"https://www.basedlabs.ai/api/generate/image/v2/{request_id}"; poll_payload = {"historyId": history_id}
        for i in range(40): # Increased polling time
            ...
            if response_poll.status_code == 200:
                ...
                print(f"BasedLabs: Poll attempt {i+1}, status: {status}")
                if status == "COMPLETED":
                    images_data = poll_data.get('history', {}).get('prediction', {}).get('images', [])
                    if images_data:
                        print(f"BasedLabs: Job completed successfully. Found {len(images_data)} image(s).")
                        return [img['url'] for img in images_data if 'url' in img]
            else:
                print(f"BasedLabs: Poll attempt {i+1} failed with status: {response_poll.status_code}")
            time.sleep(5)
...
# --- Video Services (No changes from here onwards) ---
