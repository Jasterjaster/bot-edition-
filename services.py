...
import uuid # <-- تم إضافة هذه المكتبة
...
# --- [جديد] إعدادات Digen API لتعديل الصور ---
...
# --- [مُعدل بالكامل] خدمات تعديل الصور باستخدام Digen API ---
def upload_image_for_editing(image_data, file_name="temp_image.jpg"):
    """الخطوة 1: رفع الصورة إلى Digen للحصول على رابط مرجعي."""
    print(f"Digen Step 1: Uploading image {file_name}")
    try:
        # Step 1.1: Get presigned URL
...
def start_image_editing_job(uploaded_image_url, prompt):
    """الخطوة 2: إرسال مهمة التعديل والحصول على Job ID."""
...
def poll_for_editing_result(job_id):
    """الخطوة 3: التحقق الدوري من حالة المهمة حتى تكتمل."""
    ...
    for attempt in range(40): # Poll for up to 120 seconds
        try:
