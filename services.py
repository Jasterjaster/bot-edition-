...
# --- [جديد] تعليمات النظام لمهام محددة ---
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "candidates" in response_json and response_json['candidates']:
                    ...
                    else: # Handle cases with no text part (e.g., safety block)
                        return "فشل في إنشاء استجابة.", None
                else: 
                    print(f"Response BLOCKED or invalid: {response_json}")
                    return "تم حظر الاستجابة من قبل نظام الأمان.", None
            else: 
...
# --- [جديد] دوال متخصصة تستخدم Gemini ---
...
def describe_image_with_gemini(image_base64):
    # Prompt is empty as the instruction is in the system prompt
...
def upload_image_for_editing(image_data, file_name="temp_image.jpg"):
    print(f"--- بدء عملية رفع الصورة للتعديل: {file_name} ---")
    file_size = len(image_data); content_type = "image/jpeg"
    try:
        url_step1 = "https://www.basedlabs.ai/api/upload/signed-url"
        payload_step1 = {"fileName": file_name, "contentType": content_type, "fileSize": file_size, "uploadTool": "generate2-drag-drop"}
        ...
        response_step2 = SESSION.put(signed_url, data=image_data, headers={'Content-Type': content_type}); response_step2.raise_for_status()
        print(f"--- تم رفع الصورة بنجاح. الرابط: {cdn_url} ---"); return cdn_url
...
def start_image_editing_job(uploaded_image_url, prompt):
    ...
    try:
        ...
        if job_id: print(f"   ... تم بدء المهمة بنجاح. ID: {job_id}"); return job_id
        return None
...
def poll_for_editing_result(job_id):
    ...
    for _ in range(30):
        try:
            ...
            elif status in ["FAILED", "ERROR"]: print(f"🚨 فشلت المهمة. الحالة: {status}"); return None
            time.sleep(5)
        except requests.exceptions.RequestException as e: print(f"حدث خطأ أثناء المتابعة: {e}"); return None
    print("انتهى وقت الانتظار للمهمة."); return None

# --- خدمات الفيديو (لا تغيير في المنطق الداخلي) ---

def upload_image_for_video(image_bytes, file_name):
    print("Starting image upload process for video...")
    try:
        ...
        upload_data = signed_url_response.json()
        ...
        print("Upload process for video completed successfully.")
        return {"cdnUrl": upload_data['cdnUrl'], "width": width, "height": height, "uploadId": upload_data['uploadId']}
    except Exception as e:
        print(f"Image upload for video failed: {e}")
        return None

def start_image_to_video_job(prompt, image_url, media_id):
    print("Starting IMAGE-TO-VIDEO generation request...")
    try:
        payload = {
            "prompt": prompt, "image_url": image_url, "mediaId": media_id, "width": 1024, "height": 1024, "duration": 8,
            "motion_bucket_id": 60, "generate_audio": True, "resolution": "1080p", "aspect_ratio": "auto",
            "model": {
                "id": 84, "label": "Veo3", "versionInfo": { "id": 144, "modelPath": "fal-ai/veo3/image-to-video", "type": "ImageToVideo" }
            }
        }
        response = SESSION.post("https://www.basedlabs.ai/api/generate/video", data=json.dumps(payload), headers=BASEDLABS_HEADERS)
        ...
        print(f"Image-to-Video generation start failed: {e}")
        return None

def start_text_to_video_job(prompt):
    print("Starting TEXT-TO-VIDEO generation request...")
    try:
        payload = {
            "prompt": prompt, "width": 1280, "height": 720, "duration": 12, "aspect_ratio": "16:9",
            "resolution": "720p", "fps": 24,
            "selectedModel": {
                "id": 136, "label": "Sora", "versionInfo": { "id": 169, "modelPath": "fal-ai/sora-2/text-to-video", "type": "TextToVideo" }
            }
        }
        response = SESSION.post("https://www.basedlabs.ai/api/generate/text-to-video", data=json.dumps(payload), headers=BASEDLABS_HEADERS)
        response.raise_for_status()
        data = response.json()
        request_id, history_id = data.get("request_id"), data.get("historyId")
        if not all([request_id, history_id]): return None
        ...
        print(f"Text-to-Video generation start failed: {e}")
        return None

def poll_for_video_result(request_id, history_id, cancel_event):
    print(f"Polling for video result with Request ID: {request_id}")
    ...
    for i in range(120): # Poll for up to 10 minutes
        if cancel_event.is_set():
            print(f"Cancellation signal received for job {request_id}. Stopping poll.")
            return "CANCELLED"
